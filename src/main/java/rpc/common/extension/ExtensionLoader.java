...
public final class ExtensionLoader<T> {
    ...
    public static <S> ExtensionLoader<S> getExtensionLoader(Class<S> type) {
        ...
        if (!Optional.ofNullable(extensionLoader).isPresent()) {
            ExtensionLoader<S> newLoader = new ExtensionLoader<S>(type);
            EXTENSION_LOADERS.putIfAbsent(type, newLoader);
            extensionLoader = newLoader;
        }
    ...
    private static void loadDefaultResources() throws IOException {
        ...
        File folder = new File(FULL_SERVICE_DIRECTORY);
    ...
    private static void loadResource(Map<String, Class<?>> extensionClasses, ClassLoader classLoader, URL resourceUrl) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(resourceUrl.openStream(), UTF_8))) {
            ...
            log.warn("Unable to read the file " + resourceUrl + "\n Caused by: " + e.getMessage());
        }
    }
    ...
    private T createExtension(String name) {
        ...
        if (!Optional.ofNullable(instance).isPresent()) {
            try {
                ...
                log.warn("Fail to create instance of extension " + clazz + "\n Caused by: " + e.getMessage());
            }
        }
    ...
    private void loadDirectory(Map<String, Class<?>> extensionClasses) {
        ...
        try {
            ...
            log.warn("Unable to locate the file " + fileName + "\n Caused by: " + e.getMessage());
        }
    }
